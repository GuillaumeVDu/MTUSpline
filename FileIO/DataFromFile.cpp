...
//    std::cout << "ERROR: " << dataFilename << " could not be open\n";
//    exit(EXIT_FAILURE);
//  }
	...
	: dataFile_ ( dataFileName.c_str() ), dataFileName_ ( dataFileName ), noTimeSteps_ ( 0 ), currentDataTime_ ( 0. ), currentTimeStep_ ( 0 )
{
	if ( !dataFile_.is_open() )
	{
		std::cout << "ERROR: " << dataFileName_ << " could not be open\n";
		exit ( EXIT_FAILURE );
	}
...
// 	std::cout << dataFileName_ << " : " << value << std::endl << std::flush;
	
// 	std::cout << currentDataTime_ << std::endl << std::flush;
...
// 		std::cout << currentDataTime_ << "-" << word << ";" << std::flush;
// 		value = std::atof(word.c_str());
// 		if(dataFileName_ == "././cfg/TMR_Heintz_16_10_15/threeDOF_EF_WP_WFNorm_Sust1_v2//emgFilt.sto")
// 		std::std::cout <<std::setprecision(std::numeric_limits<double>::digits10 + 1) << value << std::endl << std::flush;
...
// 		}
	...
	{
		std::cout << "ERROR: in " << dataFileName_ << " at time step " << currentTimeStep_ << " you have " << currentData_.size() << " input.\nYou need " << headerFile_.getNumberOfColumn() - 1 << endl << std::flush;
		exit ( EXIT_FAILURE );
	}
